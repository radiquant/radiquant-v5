#!/usr/bin/env python3
"""Validate the Radi144 Materialized Projection Migration File Decision Gate."""
...
DECISION_SCHEMA = ROOT / "packages" / "contracts" / "projections" / "radi144-materialized-projection-migration-file-decision.schema.v1.json"
DECISION_INSTANCE = ROOT / "packages" / "contracts" / "projections" / "radi144-materialized-projection-migration-file-decision.v1.instance.json"
PROJECT_ANCHOR = ROOT / "docs" / "pi" / "project.yml"
DECISION_DOC = ROOT / "docs" / "architecture" / "RADI144_MATERIALIZED_PROJECTION_MIGRATION_FILE_DECISION_GATE.md"
...
def main() -> int:
    ...
    if runtime_scope.get("migration_file_decision_recorded") is not True:
        fail("Radi144 migration file decision must be recorded")
    ...
    boundary = manifest.get("materialized_projection_migration_file_decision_boundary", {})
    if boundary.get("instance_path") != "packages/contracts/projections/radi144-materialized-projection-migration-file-decision.v1.instance.json":
        fail("Radi144 manifest must link migration file decision")
    if schema.get("properties", {}).get("schema_id", {}).get("const") != instance.get("schema_id"):
        fail("Radi144 migration file decision schema_id drift")
    if boundary.get("schema_id") != instance.get("schema_id"):
        fail("Radi144 migration file decision boundary schema_id drift")
    if instance.get("decision") != "defer_migration_file_creation_until_file_contract_gate":
        fail("Radi144 migration file decision must defer file creation")
    ...
    if instance.get("required_future_gate") != "radi144_materialized_projection_migration_file_contract_gate_decision":
        fail("Radi144 migration file must point to migration file contract future gate")
    for flag in ["migration_file_enabled", "alembic_revision_file_enabled", "alembic_migration_enabled", "projection_storage_tables_enabled", "orm_model_enabled", "projection_write_service_enabled", "worker_projection_materialization_enabled"]:
        if instance.get(flag) is not False or boundary.get(flag) is not False:
            fail(f"Radi144 migration file {flag} must remain false")

    preconditions = set(instance.get("file_contract_preconditions", []))
    for precondition in ["alembic_revision_implementation_decision_recorded", "planned_revision_file_path_confirmed", "upgrade_create_table_contract_reviewed", "downgrade_drop_table_contract_reviewed", "partial_unique_index_contract_reviewed", "json_column_contract_reviewed", "foreign_key_contract_reviewed", "migration_file_still_absent", "orm_model_still_blocked", "write_service_still_blocked"]:
        if precondition not in preconditions:
            fail(f"Radi144 migration file missing precondition: {precondition}")

    if "module_projections" in set(Base.metadata.tables):
        fail("module_projections table must remain absent in migration file decision gate")
    if PLANNED_MIGRATION_FILE.exists():
        fail("0008_module_projections.py must not exist in migration file decision gate")
    implementation_text = "\n".join([
    ...
    for token in [
        ...
        if token not in project_text:
            fail(f"Project anchor missing migration file token: {token}")
    ...
    for token in ["Planned migration file", "apps/api/alembic/versions/0008_module_projections.py", "radi144_materialized_projection_migration_file_contract_gate_decision"]:
        if token not in decision_text:
            fail(f"Migration file doc missing token: {token}")

    print("[OK] Radi144 materialized projection migration file decision is recorded")
