#!/usr/bin/env python3
"""Validate the Radi144 Materialized Projection Migration File Repository File Review Implementation Decision Gate."""
...
SCHEMA = ROOT / "packages" / "contracts" / "projections" / "radi144-materialized-projection-migration-file-repository-file-review-implementation-decision.schema.v1.json"
INSTANCE = ROOT / "packages" / "contracts" / "projections" / "radi144-materialized-projection-migration-file-repository-file-review-implementation-decision.v1.instance.json"
PROJECT_ANCHOR = ROOT / "docs" / "pi" / "project.yml"
DECISION_DOC = ROOT / "docs" / "architecture" / "RADI144_MATERIALIZED_PROJECTION_MIGRATION_FILE_REPOSITORY_FILE_REVIEW_IMPLEMENTATION_DECISION_GATE.md"
...
REQUIRED_PRECONDITIONS = {
    ...
    "repository_file_review_still_deferred",
...
def main() -> int:
    ...
    boundary = manifest.get("materialized_projection_migration_file_repository_file_review_implementation_decision_boundary", {})

    if runtime_scope.get("migration_file_repository_file_review_implementation_decision_recorded") is not True:
        fail("Radi144 migration file repository file review implementation decision must be recorded")
    if boundary.get("instance_path") != "packages/contracts/projections/radi144-materialized-projection-migration-file-repository-file-review-implementation-decision.v1.instance.json":
        fail("Radi144 manifest must link migration file repository file review implementation decision")
    if schema.get("properties", {}).get("schema_id", {}).get("const") != instance.get("schema_id") or boundary.get("schema_id") != instance.get("schema_id"):
        fail("Radi144 migration file repository file review implementation schema_id drift")
    if instance.get("decision") != "defer_migration_file_repository_file_review_implementation_until_repository_file_acceptance_gate":
        fail("Radi144 migration file repository file review implementation must be deferred")
    if instance.get("required_future_gate") != "radi144_materialized_projection_migration_file_repository_file_acceptance_gate_decision":
        fail("Radi144 migration file repository file review implementation must point to acceptance future gate")
    ...
    for flag in schema.get("required", []):
        if flag.endswith("_allowed") or flag.endswith("_enabled"):
            if instance.get(flag) is not False or boundary.get(flag) is not False:
                fail(f"Radi144 migration file repository file review implementation {flag} must remain false")
    ...
    preconditions = set(instance.get("repository_file_review_implementation_preconditions", []))
    if not REQUIRED_PRECONDITIONS.issubset(preconditions):
        fail(f"Radi144 migration file repository file review implementation missing preconditions: {sorted(REQUIRED_PRECONDITIONS - preconditions)}")

    if "module_projections" in set(Base.metadata.tables):
        fail("module_projections table must remain absent in review implementation decision gate")
    if PLANNED_MIGRATION_FILE.exists():
        fail("0008_module_projections.py must not exist in review implementation decision gate")
    implementation_text = "\n".join([
    ...
    for token in [
        ...
        "status: migration_file_repository_file_review_implementation_deferred_no_file",
        "radi144_materialized_projection_migration_file_repository_file_acceptance_gate_decision",
        "safe_bundling_only_within_single_safety_boundary",
    ]:
        if token not in project_text:
            fail(f"Project anchor missing migration file repository file review implementation token: {token}")
    ...
    for token in ["Planned repository file review implementation target", "repository file review implementation remains deferred", "radi144_materialized_projection_migration_file_repository_file_acceptance_gate_decision"]:
        if token not in decision_text:
            fail(f"Migration file repository file review implementation doc missing token: {token}")

    print("[OK] Radi144 materialized projection migration file repository file review implementation decision is recorded")
