#!/usr/bin/env python3
"""Validate the Radi144 Materialized Projection Migration File Authoring Decision Gate."""
...
DECISION_SCHEMA = ROOT / "packages" / "contracts" / "projections" / "radi144-materialized-projection-migration-file-authoring-decision.schema.v1.json"
DECISION_INSTANCE = ROOT / "packages" / "contracts" / "projections" / "radi144-materialized-projection-migration-file-authoring-decision.v1.instance.json"
PROJECT_ANCHOR = ROOT / "docs" / "pi" / "project.yml"
DECISION_DOC = ROOT / "docs" / "architecture" / "RADI144_MATERIALIZED_PROJECTION_MIGRATION_FILE_AUTHORING_DECISION_GATE.md"
...
REQUIRED_PRECONDITIONS = {
    ...
    "revision_identifiers_confirmed",
    "required_imports_confirmed",
    "upgrade_steps_confirmed",
    "downgrade_steps_confirmed",
    "forbidden_content_confirmed",
...
def main() -> int:
    ...
    if runtime_scope.get("migration_file_authoring_decision_recorded") is not True:
        fail("Radi144 migration file authoring decision must be recorded")
    ...
    boundary = manifest.get("materialized_projection_migration_file_authoring_decision_boundary", {})
    if boundary.get("instance_path") != "packages/contracts/projections/radi144-materialized-projection-migration-file-authoring-decision.v1.instance.json":
        fail("Radi144 manifest must link migration file authoring decision")
    if schema.get("properties", {}).get("schema_id", {}).get("const") != instance.get("schema_id"):
        fail("Radi144 migration file authoring decision schema_id drift")
    if boundary.get("schema_id") != instance.get("schema_id"):
        fail("Radi144 migration file authoring decision boundary schema_id drift")
    if instance.get("decision") != "defer_migration_file_authoring_until_write_gate":
        fail("Radi144 migration file authoring must be deferred until write gate")
    ...
    if instance.get("required_future_gate") != "radi144_materialized_projection_migration_file_write_gate_decision":
        fail("Radi144 migration file authoring must point to migration file write future gate")
    ...
    for flag in ["migration_file_enabled", "alembic_revision_file_enabled", "alembic_migration_enabled", "projection_storage_tables_enabled", "orm_model_enabled", "projection_write_service_enabled", "worker_projection_materialization_enabled"]:
        if instance.get(flag) is not False or boundary.get(flag) is not False:
            fail(f"Radi144 migration file authoring {flag} must remain false")

    preconditions = set(instance.get("authoring_preconditions", []))
    if not REQUIRED_PRECONDITIONS.issubset(preconditions):
        fail(f"Radi144 migration file authoring missing preconditions: {sorted(REQUIRED_PRECONDITIONS - preconditions)}")

    if "module_projections" in set(Base.metadata.tables):
        fail("module_projections table must remain absent in migration file authoring decision gate")
    if PLANNED_MIGRATION_FILE.exists():
        fail("0008_module_projections.py must not exist in migration file authoring decision gate")
    implementation_text = "\n".join([
    ...
    for token in [
        ...
        if token not in project_text:
            fail(f"Project anchor missing migration file authoring token: {token}")
    ...
    for token in ["Planned authoring target", "authoring deferred", "radi144_materialized_projection_migration_file_write_gate_decision"]:
        if token not in decision_text:
            fail(f"Migration file authoring doc missing token: {token}")

    print("[OK] Radi144 materialized projection migration file authoring decision is recorded")
