#!/usr/bin/env python3
"""Validate the Radi144 Materialized Projection Migration File Repository File Review Decision Gate."""
...
SCHEMA = ROOT / "packages" / "contracts" / "projections" / "radi144-materialized-projection-migration-file-repository-file-review-decision.schema.v1.json"
INSTANCE = ROOT / "packages" / "contracts" / "projections" / "radi144-materialized-projection-migration-file-repository-file-review-decision.v1.instance.json"
PROJECT_ANCHOR = ROOT / "docs" / "pi" / "project.yml"
DECISION_DOC = ROOT / "docs" / "architecture" / "RADI144_MATERIALIZED_PROJECTION_MIGRATION_FILE_REPOSITORY_FILE_REVIEW_DECISION_GATE.md"
...
REQUIRED_PRECONDITIONS = {
    ...
    "repository_file_access_implementation_still_deferred",
...
def main() -> int:
    ...
    boundary = manifest.get("materialized_projection_migration_file_repository_file_review_decision_boundary", {})
    runtime_scope = manifest.get("runtime_scope", {})

    if runtime_scope.get("migration_file_repository_file_review_decision_recorded") is not True:
        fail("Radi144 migration file repository file review decision must be recorded")
    if boundary.get("instance_path") != "packages/contracts/projections/radi144-materialized-projection-migration-file-repository-file-review-decision.v1.instance.json":
        fail("Radi144 manifest must link migration file repository file review decision")
    if schema.get("properties", {}).get("schema_id", {}).get("const") != instance.get("schema_id") or boundary.get("schema_id") != instance.get("schema_id"):
        fail("Radi144 migration file repository file review schema_id drift")
    if instance.get("decision") != "defer_migration_file_repository_file_review_until_repository_file_review_implementation_gate":
        fail("Radi144 migration file repository file review must be deferred")
    if instance.get("required_future_gate") != "radi144_materialized_projection_migration_file_repository_file_review_implementation_gate_decision":
        fail("Radi144 migration file repository file review must point to review implementation future gate")
    ...
    for flag in schema.get("required", []):
        if flag.endswith("_allowed") or flag.endswith("_enabled"):
            if instance.get(flag) is not False or boundary.get(flag) is not False:
                fail(f"Radi144 migration file repository file review {flag} must remain false")
    ...
    preconditions = set(instance.get("repository_file_review_preconditions", []))
    if not REQUIRED_PRECONDITIONS.issubset(preconditions):
        fail(f"Radi144 migration file repository file review missing preconditions: {sorted(REQUIRED_PRECONDITIONS - preconditions)}")

    if "module_projections" in set(Base.metadata.tables):
        fail("module_projections table must remain absent in review decision gate")
    if PLANNED_MIGRATION_FILE.exists():
        fail("0008_module_projections.py must not exist in review decision gate")
    implementation_text = "\n".join([
    ...
    for token in [
        ...
        "status: migration_file_repository_file_review_deferred_no_file",
        ...
        if token not in project_text:
            fail(f"Project anchor missing migration file repository file review token: {token}")
    ...
    for token in ["Planned repository file review target", "repository file review remains deferred", "radi144_materialized_projection_migration_file_repository_file_review_implementation_gate_decision"]:
        if token not in decision_text:
            fail(f"Migration file repository file review doc missing token: {token}")

    print("[OK] Radi144 materialized projection migration file repository file review decision is recorded")
