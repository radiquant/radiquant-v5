#!/usr/bin/env python3
"""Validate the Radi144 Materialized Projection Migration File Repository File Opening Decision Gate."""
...
DECISION_SCHEMA = ROOT / "packages" / "contracts" / "projections" / "radi144-materialized-projection-migration-file-repository-file-opening-decision.schema.v1.json"
DECISION_INSTANCE = ROOT / "packages" / "contracts" / "projections" / "radi144-materialized-projection-migration-file-repository-file-opening-decision.v1.instance.json"
PROJECT_ANCHOR = ROOT / "docs" / "pi" / "project.yml"
DECISION_DOC = ROOT / "docs" / "architecture" / "RADI144_MATERIALIZED_PROJECTION_MIGRATION_FILE_REPOSITORY_FILE_OPENING_DECISION_GATE.md"
...
REQUIRED_PRECONDITIONS = {
    ...
    "repository_file_activation_implementation_still_deferred",
...
def main() -> int:
    ...
    if runtime_scope.get("migration_file_repository_file_opening_decision_recorded") is not True:
        fail("Radi144 migration file repository file opening decision must be recorded")
    ...
    boundary = manifest.get("materialized_projection_migration_file_repository_file_opening_decision_boundary", {})
    if boundary.get("instance_path") != "packages/contracts/projections/radi144-materialized-projection-migration-file-repository-file-opening-decision.v1.instance.json":
        fail("Radi144 manifest must link migration file repository file opening decision")
    if schema.get("properties", {}).get("schema_id", {}).get("const") != instance.get("schema_id"):
        fail("Radi144 migration file repository file opening decision schema_id drift")
    if boundary.get("schema_id") != instance.get("schema_id"):
        fail("Radi144 migration file repository file opening decision boundary schema_id drift")
    if instance.get("decision") != "defer_migration_file_repository_file_opening_until_repository_file_opening_implementation_gate":
        fail("Radi144 migration file repository file opening must be deferred until repository file opening implementation gate")
    ...
    if instance.get("required_future_gate") != "radi144_materialized_projection_migration_file_repository_file_opening_implementation_gate_decision":
        fail("Radi144 migration file repository file opening must point to repository file opening implementation future gate")
    if instance.get("migration_file_repository_file_opening_allowed") is not False or boundary.get("migration_file_repository_file_opening_allowed") is not False:
        fail("Radi144 migration file repository file opening must remain disallowed")
    for flag in [
        ...
        if instance.get(flag) is not False or boundary.get(flag) is not False:
            fail(f"Radi144 migration file repository file opening {flag} must remain false")

    preconditions = set(instance.get("repository_file_opening_preconditions", []))
    if not REQUIRED_PRECONDITIONS.issubset(preconditions):
        fail(f"Radi144 migration file repository file opening missing preconditions: {sorted(REQUIRED_PRECONDITIONS - preconditions)}")

    if "module_projections" in set(Base.metadata.tables):
        fail("module_projections table must remain absent in migration file repository file opening decision gate")
    if PLANNED_MIGRATION_FILE.exists():
        fail("0008_module_projections.py must not exist in migration file repository file opening decision gate")
    implementation_text = "\n".join([
    ...
    for token in [
        ...
        "status: migration_file_repository_file_opening_deferred_no_file",
        ...
        if token not in project_text:
            fail(f"Project anchor missing migration file repository file opening token: {token}")
    ...
    for token in ["Planned repository file opening target", "repository file opening remains deferred", "radi144_materialized_projection_migration_file_repository_file_opening_implementation_gate_decision"]:
        if token not in decision_text:
            fail(f"Migration file repository file opening doc missing token: {token}")

    print("[OK] Radi144 materialized projection migration file repository file opening decision is recorded")
