#!/usr/bin/env python3
"""Validate the Radi144 Materialized Projection Migration File Write Implementation Decision Gate."""
...
DECISION_SCHEMA = ROOT / "packages" / "contracts" / "projections" / "radi144-materialized-projection-migration-file-write-implementation-decision.schema.v1.json"
DECISION_INSTANCE = ROOT / "packages" / "contracts" / "projections" / "radi144-materialized-projection-migration-file-write-implementation-decision.v1.instance.json"
PROJECT_ANCHOR = ROOT / "docs" / "pi" / "project.yml"
DECISION_DOC = ROOT / "docs" / "architecture" / "RADI144_MATERIALIZED_PROJECTION_MIGRATION_FILE_WRITE_IMPLEMENTATION_DECISION_GATE.md"
...
def main() -> int:
    ...
    if runtime_scope.get("migration_file_write_implementation_decision_recorded") is not True:
        fail("Radi144 migration file write implementation decision must be recorded")
    ...
    boundary = manifest.get("materialized_projection_migration_file_write_implementation_decision_boundary", {})
    if boundary.get("instance_path") != "packages/contracts/projections/radi144-materialized-projection-migration-file-write-implementation-decision.v1.instance.json":
        fail("Radi144 manifest must link migration file write implementation decision")
    if schema.get("properties", {}).get("schema_id", {}).get("const") != instance.get("schema_id"):
        fail("Radi144 migration file write implementation decision schema_id drift")
    if boundary.get("schema_id") != instance.get("schema_id"):
        fail("Radi144 migration file write implementation decision boundary schema_id drift")
    if instance.get("decision") != "defer_migration_file_write_implementation_until_file_introduction_gate":
        fail("Radi144 migration file write implementation must be deferred until file introduction gate")
    ...
    if instance.get("required_future_gate") != "radi144_materialized_projection_migration_file_introduction_gate_decision":
        fail("Radi144 migration file write implementation must point to migration file introduction future gate")
    ...
    for flag in ["migration_file_enabled", "alembic_revision_file_enabled", "alembic_migration_enabled", "projection_storage_tables_enabled", "orm_model_enabled", "projection_write_service_enabled", "worker_projection_materialization_enabled"]:
        if instance.get(flag) is not False or boundary.get(flag) is not False:
            fail(f"Radi144 migration file write implementation {flag} must remain false")

    preconditions = set(instance.get("implementation_preconditions", []))
    if not REQUIRED_PRECONDITIONS.issubset(preconditions):
        fail(f"Radi144 migration file write implementation missing preconditions: {sorted(REQUIRED_PRECONDITIONS - preconditions)}")

    if "module_projections" in set(Base.metadata.tables):
        fail("module_projections table must remain absent in migration file write implementation decision gate")
    if PLANNED_MIGRATION_FILE.exists():
        fail("0008_module_projections.py must not exist in migration file write implementation decision gate")
    implementation_text = "\n".join([
    ...
    for token in [
        ...
        if token not in project_text:
            fail(f"Project anchor missing migration file write implementation token: {token}")
    ...
    for token in ["Planned write implementation target", "write implementation deferred", "radi144_materialized_projection_migration_file_introduction_gate_decision"]:
        if token not in decision_text:
            fail(f"Migration file write implementation doc missing token: {token}")

    print("[OK] Radi144 materialized projection migration file write implementation decision is recorded")
