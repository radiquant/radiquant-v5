#!/usr/bin/env python3
"""Validate the Radi144 Materialized Projection Migration File Repository File Acceptance Implementation Decision Gate."""
...
SCHEMA = ROOT / "packages" / "contracts" / "projections" / "radi144-materialized-projection-migration-file-repository-file-acceptance-implementation-decision.schema.v1.json"
INSTANCE = ROOT / "packages" / "contracts" / "projections" / "radi144-materialized-projection-migration-file-repository-file-acceptance-implementation-decision.v1.instance.json"
PROJECT_ANCHOR = ROOT / "docs" / "pi" / "project.yml"
DECISION_DOC = ROOT / "docs" / "architecture" / "RADI144_MATERIALIZED_PROJECTION_MIGRATION_FILE_REPOSITORY_FILE_ACCEPTANCE_IMPLEMENTATION_DECISION_GATE.md"
...
REQUIRED_PRECONDITIONS = {
    ...
    "repository_file_acceptance_still_deferred",
...
def main() -> int:
    ...
    boundary = manifest.get("materialized_projection_migration_file_repository_file_acceptance_implementation_decision_boundary", {})

    if runtime_scope.get("migration_file_repository_file_acceptance_implementation_decision_recorded") is not True:
        fail("Radi144 migration file repository file acceptance implementation decision must be recorded")
    if boundary.get("instance_path") != "packages/contracts/projections/radi144-materialized-projection-migration-file-repository-file-acceptance-implementation-decision.v1.instance.json":
        fail("Radi144 manifest must link migration file repository file acceptance implementation decision")
    ...
    if instance.get("decision") != "defer_migration_file_repository_file_acceptance_implementation_until_repository_file_admission_gate":
        fail("Radi144 migration file repository file acceptance implementation must be deferred")
    if instance.get("required_future_gate") != "radi144_materialized_projection_migration_file_repository_file_admission_gate_decision":
    ...
    preconditions = set(instance.get("repository_file_acceptance_implementation_preconditions", []))
    ...
    if "module_projections" in set(Base.metadata.tables):
        fail("module_projections table must remain absent in acceptance implementation decision gate")
    if PLANNED_MIGRATION_FILE.exists():
        fail("0008_module_projections.py must not exist in acceptance implementation decision gate")
    ...
    for token in ["radi144_materialized_projection_migration_file_repository_file_acceptance_implementation_gate_decision", "status: migration_file_repository_file_acceptance_implementation_deferred_no_file", "radi144_materialized_projection_migration_file_repository_file_admission_gate_decision", "safe_bundling_only_within_single_safety_boundary"]:
        if token not in project_text:
            fail(f"Project anchor missing migration file repository file acceptance token: {token}")
    for token in ["Planned repository file acceptance implementation target", "repository file acceptance implementation remains deferred", "radi144_materialized_projection_migration_file_repository_file_admission_gate_decision"]:
        if token not in decision_text:
            fail(f"Migration file repository file acceptance doc missing token: {token}")

    print("[OK] Radi144 materialized projection migration file repository file acceptance implementation decision is recorded")
