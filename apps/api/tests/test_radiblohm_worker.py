"""RadiBlohm worker runtime tests."""
...
from app.models.radiblohm import RadiBlohmResult  # noqa: E402
...
from app.services.radiblohm.worker_runtime import (  # noqa: E402
...
async def radiblohm_factory() -> AsyncIterator[async_sessionmaker[AsyncSession]]:
...
async def test_job_record_create_is_idempotent(
    radiblohm_factory: async_sessionmaker[AsyncSession],
) -> None:
    tenant_id, session_id, workflow_run_id, step_id, _client_id = await _seed_context(
        radiblohm_factory
    )

    async with radiblohm_factory() as session:
        service = RadiBlohmJobRecordService(session)
...
async def test_worker_processes_queued_job(
    ...
    tenant_id, module_run_id = await _create_job(radiblohm_factory)

    async with radiblohm_factory() as session:
        outcome = await RadiBlohmWorkerRuntimeService(
            session,
...
async def test_worker_status_check_reads_stored_job(
    ...
    async with radiblohm_factory() as session:
        ...
        stored = await RadiBlohmJobRecordService(session).get_job_record(
            tenant_id=tenant_id,
...
async def test_worker_filters_by_tenant(
    ...
    tenant_id, _module_run_id = await _create_job(radiblohm_factory)
...
async def test_worker_second_run_is_idempotent_no_duplicate_result(
    ...
    async with radiblohm_factory() as session:
        service = RadiBlohmWorkerRuntimeService(session, engine=RadiBlohmEngine(seed=123))
        first = await service.process_next_queued(tenant_id=tenant_id)
        second = await service.process_next_queued(tenant_id=tenant_id)
        result_count = await session.scalar(select(func.count()).select_from(RadiBlohmResult))
