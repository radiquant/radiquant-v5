...
import app.models.radiblohm  # noqa: F401, E402
...
from app.routes.radiblohm import router as radiblohm_router  # noqa: E402
from app.services.auth import AuthService, get_auth_service  # noqa: E402
from app.services.radiblohm.engine import RadiBlohmEngine  # noqa: E402
from app.services.radiblohm.worker_runtime import RadiBlohmWorkerRuntimeService  # noqa: E402


@pytest_asyncio.fixture
async def radiblohm_api() -> AsyncIterator[
    ...
    auth_service = AuthService(
        settings=Settings(
            ...
            SECRET_KEY=SecretStr("radiblohm-api-secret-minimum-32-characters"),
    ...
    app.include_router(radiblohm_router)
...
async def test_create_job_returns_status(
    radiblohm_api: tuple[FastAPI, async_sessionmaker[AsyncSession], AuthService],
) -> None:
    app, factory, auth_service = radiblohm_api
...
async def test_get_job_status_returns_tenant_job(
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        response = await client.get(
            f"/engines/radiblohm/jobs/{job_id}",
            headers=_headers(auth_service, tenant_id, user_id),
        )
...
async def test_result_client_projection_hides_raw_payload(
    ...
    async with factory() as session:
        await RadiBlohmWorkerRuntimeService(
            session,
            engine=RadiBlohmEngine(seed=123),
        ).process_next_queued(tenant_id=tenant_id)
        await session.commit()

    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        response = await client.get(
            f"/engines/radiblohm/jobs/{job_id}/result?role=client",
            headers=_headers(auth_service, tenant_id, user_id),
        )
    ...
    assert "superposition_terms" not in body
...
async def test_result_therapist_projection_contains_details(
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        response = await client.get(
            f"/engines/radiblohm/jobs/{job_id}/result?role=therapist",
            headers=_headers(auth_service, tenant_id, user_id),
        )
    ...
    assert body["morphic_field"]["coherence"] >= 0
    assert body["geometry"]["form"]
    assert body["tcm"]["element"]
...
async def test_wrong_tenant_gets_404_for_result(
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        response = await client.get(
            f"/engines/radiblohm/jobs/{job_id}/result",
