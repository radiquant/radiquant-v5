...
from app.routes.radiworks import router as radiworks_router  # noqa: E402
...
from app.services.radiworks.worker_runtime import RadiWorksWorkerRuntimeService  # noqa: E402


@pytest_asyncio.fixture
async def radiworks_api() -> AsyncIterator[
    ...
    auth_service = AuthService(
        settings=Settings(
            ...
            SECRET_KEY=SecretStr("radiworks-api-secret-minimum-32-characters"),
    ...
    app.include_router(radiworks_router)
...
async def test_create_job_returns_status(
    radiworks_api: tuple[FastAPI, async_sessionmaker[AsyncSession], AuthService],
) -> None:
    app, factory, auth_service = radiworks_api
...
async def test_get_job_status_returns_tenant_job(
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        response = await client.get(
            f"/engines/radiworks/jobs/{job_id}",
            headers=_headers(auth_service, tenant_id, user_id),
        )
...
async def test_result_client_projection_hides_raw_payload(
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        response = await client.get(
            f"/engines/radiworks/jobs/{job_id}/result?role=client",
            headers=_headers(auth_service, tenant_id, user_id),
        )
...
async def test_result_therapist_projection_contains_details(
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        response = await client.get(
            f"/engines/radiworks/jobs/{job_id}/result?role=therapist",
            headers=_headers(auth_service, tenant_id, user_id),
        )
    ...
    assert body["monte_carlo"]["trials"] > 0
    assert body["rates"]
...
async def test_wrong_tenant_gets_404_for_result(
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        response = await client.get(
            f"/engines/radiworks/jobs/{job_id}/result",
