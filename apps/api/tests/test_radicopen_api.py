...
import app.models.radicopen  # noqa: F401, E402
...
from app.routes.radicopen import router as radicopen_router  # noqa: E402
from app.services.auth import AuthService, get_auth_service  # noqa: E402
from app.services.radicopen.engine import RadiCopenEngine  # noqa: E402
from app.services.radicopen.worker_runtime import RadiCopenWorkerRuntimeService  # noqa: E402


@pytest_asyncio.fixture
async def radicopen_api() -> AsyncIterator[
    ...
    auth_service = AuthService(
        settings=Settings(
            ...
            SECRET_KEY=SecretStr("radicopen-api-secret-minimum-32-characters"),
    ...
    app.include_router(radicopen_router)
    ...
    *,
    ...
    async with factory() as session:
        ...
        goal = SessionGoal(
            tenant_id=tenant.id,
            client_id=client.id,
            title="Morphic Balance",
            description="Centered Copen session",
            created_by_user_id=user.id,
        )
        session.add(goal)
        await session.flush()
...
async def test_create_job_returns_status(
    radicopen_api: tuple[FastAPI, async_sessionmaker[AsyncSession], AuthService],
) -> None:
    app, factory, auth_service = radicopen_api
...
async def test_get_job_status_returns_tenant_job(
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        response = await client.get(
            f"/engines/radicopen/jobs/{job_id}",
            headers=_headers(auth_service, tenant_id, user_id),
        )
...
async def test_result_client_projection_hides_raw_payload(
    ...
    async with factory() as session:
        await RadiCopenWorkerRuntimeService(
            session,
            engine=RadiCopenEngine(seed=123),
        ).process_next_queued(tenant_id=tenant_id)
        await session.commit()

    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        response = await client.get(
            f"/engines/radicopen/jobs/{job_id}/result?role=client",
            headers=_headers(auth_service, tenant_id, user_id),
        )
    ...
    assert "level_resonance" not in body
...
async def test_result_therapist_projection_contains_details(
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        response = await client.get(
            f"/engines/radicopen/jobs/{job_id}/result?role=therapist",
            headers=_headers(auth_service, tenant_id, user_id),
        )
    ...
    assert body["rate"]["final_rate"] > 0
    assert len(body["level_resonance"]["levels"]) == 13
    assert body["dominant_level"] >= 1
...
async def test_wrong_tenant_gets_404_for_result(
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        response = await client.get(
            f"/engines/radicopen/jobs/{job_id}/result",
