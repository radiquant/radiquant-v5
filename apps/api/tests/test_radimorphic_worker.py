"""RadiMorphic worker runtime tests."""
...
from app.models.radimorphic import RadiMorphicResult  # noqa: E402
...
async def radimorphic_factory() -> AsyncIterator[async_sessionmaker[AsyncSession]]:
...
async def test_job_record_create_is_idempotent(
    radimorphic_factory: async_sessionmaker[AsyncSession],
) -> None:
    tenant_id, session_id, workflow_run_id, step_id, _client_id = await _seed_context(
        radimorphic_factory
    )

    async with radimorphic_factory() as session:
        service = RadiMorphicJobRecordService(session)
...
async def test_worker_processes_queued_job(
    ...
    tenant_id, module_run_id = await _create_job(radimorphic_factory)

    async with radimorphic_factory() as session:
        outcome = await RadiMorphicWorkerRuntimeService(
            session,
...
async def test_worker_status_check_reads_stored_job(
    ...
    async with radimorphic_factory() as session:
        ...
        stored = await RadiMorphicJobRecordService(session).get_job_record(
            tenant_id=tenant_id,
...
async def test_worker_filters_by_tenant(
    ...
    tenant_id, _module_run_id = await _create_job(radimorphic_factory)
...
async def test_worker_second_run_is_idempotent_no_duplicate_result(
    ...
    async with radimorphic_factory() as session:
        service = RadiMorphicWorkerRuntimeService(session, engine=RadiMorphicEngine(seed=123))
        first = await service.process_next_queued(tenant_id=tenant_id)
        second = await service.process_next_queued(tenant_id=tenant_id)
        result_count = await session.scalar(select(func.count()).select_from(RadiMorphicResult))
