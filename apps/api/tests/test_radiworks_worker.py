"""RadiWorks worker runtime tests."""
...
import app.models.radiworks  # noqa: F401, E402
...
from app.models.radiworks import RadiWorksResult  # noqa: E402
...
from app.services.radiworks.engine import RadiWorksEngine  # noqa: E402
from app.services.radiworks.worker_runtime import (  # noqa: E402
...
async def radiworks_factory() -> AsyncIterator[async_sessionmaker[AsyncSession]]:
...
async def test_job_record_create_is_idempotent(
    radiworks_factory: async_sessionmaker[AsyncSession],
) -> None:
    tenant_id, session_id, workflow_run_id, step_id, _client_id = await _seed_context(
        radiworks_factory
    )

    async with radiworks_factory() as session:
        service = RadiWorksJobRecordService(session)
...
async def test_worker_processes_queued_job(
    ...
    tenant_id, module_run_id = await _create_job(radiworks_factory)

    async with radiworks_factory() as session:
        outcome = await RadiWorksWorkerRuntimeService(
            session,
...
async def test_worker_status_check_reads_stored_job(
    ...
    async with radiworks_factory() as session:
        await RadiWorksWorkerRuntimeService(
            session,
            engine=RadiWorksEngine(seed=123),
        ).process_next_queued(tenant_id=tenant_id)
        stored = await RadiWorksJobRecordService(session).get_job_record(
            tenant_id=tenant_id,
...
async def test_worker_filters_by_tenant(
    ...
    tenant_id, _module_run_id = await _create_job(radiworks_factory)
...
async def test_worker_second_run_is_idempotent_no_duplicate_result(
    ...
    async with radiworks_factory() as session:
        service = RadiWorksWorkerRuntimeService(session, engine=RadiWorksEngine(seed=123))
        first = await service.process_next_queued(tenant_id=tenant_id)
        second = await service.process_next_queued(tenant_id=tenant_id)
        result_count = await session.scalar(select(func.count()).select_from(RadiWorksResult))
