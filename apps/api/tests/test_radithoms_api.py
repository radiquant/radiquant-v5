...
from app.routes.radithoms import router as radithoms_router  # noqa: E402
...
from app.services.radithoms.worker_runtime import RadiThomsWorkerRuntimeService  # noqa: E402


@pytest_asyncio.fixture
async def radithoms_api() -> AsyncIterator[
    ...
    auth_service = AuthService(
        settings=Settings(
            ...
            SECRET_KEY=SecretStr("radithoms-api-secret-minimum-32-characters"),
    ...
    app.include_router(radithoms_router)
...
async def test_create_job_returns_status(
    radithoms_api: tuple[FastAPI, async_sessionmaker[AsyncSession], AuthService],
) -> None:
    app, factory, auth_service = radithoms_api
...
async def test_get_job_status_returns_tenant_job(
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        response = await client.get(
            f"/engines/radithoms/jobs/{job_id}",
            headers=_headers(auth_service, tenant_id, user_id),
        )
...
async def test_result_client_projection_hides_raw_payload(
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        response = await client.get(
            f"/engines/radithoms/jobs/{job_id}/result?role=client",
            headers=_headers(auth_service, tenant_id, user_id),
        )
    ...
    assert "client_vector" not in body
...
async def test_result_therapist_projection_contains_details(
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        response = await client.get(
            f"/engines/radithoms/jobs/{job_id}/result?role=therapist",
            headers=_headers(auth_service, tenant_id, user_id),
        )
    ...
    assert len(body["meridian_balances"]) == 12
    assert body["recommended_points"]
    assert body["attractor"]["current_attractor"]
...
async def test_wrong_tenant_gets_404_for_result(
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        response = await client.get(
            f"/engines/radithoms/jobs/{job_id}/result",
