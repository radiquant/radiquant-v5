"""RadiWorks result storage ORM models."""
...
class RadiWorksResult(TimestampMixin, Base):
    """Tenant-scoped sensitive RadiWorks result payload."""

    __tablename__ = "radiworks_results"
    ...
    __table_args__ = (
        CheckConstraint("schema_id = 'radiworks_result_v1'", name="radiworks_results_schema_v1"),
        CheckConstraint(
            "projection_status = 'pending_projection_builder'",
            name="radiworks_results_projection_pending",
        ),
        Index("ix_radiworks_results_tenant_run", "tenant_id", "module_run_id"),
    )
