"""RadiBlohm result storage ORM models."""
...
class RadiBlohmResult(TimestampMixin, Base):
    """Tenant-scoped sensitive RadiBlohm result payload."""

    __tablename__ = "radiblohm_results"
    ...
    __table_args__ = (
        CheckConstraint("schema_id = 'radiblohm_result_v1'", name="radiblohm_results_schema_v1"),
        CheckConstraint(
            "projection_status = 'pending_projection_builder'",
            name="radiblohm_results_projection_pending",
        ),
        Index("ix_radiblohm_results_tenant_run", "tenant_id", "module_run_id"),
    )
