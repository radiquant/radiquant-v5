"""RadiCopen result storage ORM models."""
...
class RadiCopenResult(TimestampMixin, Base):
    """Tenant-scoped sensitive RadiCopen result payload."""

    __tablename__ = "radicopen_results"
    ...
    __table_args__ = (
        CheckConstraint("schema_id = 'radicopen_result_v1'", name="radicopen_results_schema_v1"),
        CheckConstraint(
            "projection_status = 'pending_projection_builder'",
            name="radicopen_results_projection_pending",
        ),
        Index("ix_radicopen_results_tenant_run", "tenant_id", "module_run_id"),
    )
