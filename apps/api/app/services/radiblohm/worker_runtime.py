"""RadiBlohm worker runtime and job-record services."""
...
from app.schemas.radiblohm import RadiBlohmJobStatus
...
class RadiBlohmJobRecordError(ValueError):
    """Raised when a RadiBlohm job record cannot be created safely."""

    public_detail = "RadiBlohm job record rejected"
...
class RadiBlohmWorkerOutcome:
    """Public-safe RadiBlohm worker runtime outcome."""

    status: RadiBlohmWorkerStatus
...
class RadiBlohmJobRecordService:
    """Create/read RadiBlohm ModuleRun job records without executing work."""
        ...
        *,
        ...
        workflow_step = await self.session.scalar(
            ...
            .where(
                ...
                WorkflowStepRun.module_id == "radiblohm",
        ...
        module_run = ModuleRun(
            ...
            job_contract_schema_id="radiblohm_engine_job_v1",
        )
    ...
    async def get_job_record(self, *, tenant_id: UUID, job_id: UUID) -> ModuleRun | None:
        """Return a tenant-scoped RadiBlohm job record by ModuleRun id."""
...
class RadiBlohmWorkerRuntimeService:
    """Process queued RadiBlohm jobs using CPU-only execution."""

    def __init__(
        self,
        session: AsyncSession,
        *,
        engine: RadiBlohmEngine | None = None,
    ) -> None:
        self.session = session
        self.engine = engine or RadiBlohmEngine()
    ...
    async def process_next_queued(
        ...
        *,
        ...
        """Process the next queued RadiBlohm job without committing."""
        statement = (
        ...
        if row is None:
            return RadiBlohmWorkerOutcome(
                status="no_queued_job",
                module_run_id=None,
                tenant_id=tenant_id,
                reason="no_queued_radiblohm_job",
            )
        ...
        try:
            await RadiBlohmResultWriter(self.session).persist_result(result=result)
        except RadiBlohmResultWriteError:
...
def job_status(module_run: ModuleRun) -> RadiBlohmJobStatus:
    """Cast a ModuleRun status into the public RadiBlohm job status type."""
