"""RadiMorphic worker runtime and job-record services."""
...
from app.schemas.radimorphic import RadiMorphicJobStatus
...
RadiMorphicWorkerStatus = Literal[
    "no_queued_job",
...
class RadiMorphicJobRecordError(ValueError):
    """Raised when a RadiMorphic job record cannot be created safely."""

    public_detail = "RadiMorphic job record rejected"
...
class RadiMorphicWorkerOutcome:
    """Public-safe RadiMorphic worker runtime outcome."""

    status: RadiMorphicWorkerStatus
...
class RadiMorphicJobRecordService:
    """Create/read RadiMorphic ModuleRun job records without executing work."""
        ...
        *,
        ...
        workflow_step = await self.session.scalar(
            ...
            .where(
                ...
                WorkflowStepRun.module_id == "radimorphic",
        ...
        if workflow_step is None:
            raise RadiMorphicJobRecordError("workflow_step_not_found_for_tenant")
        ...
        module_run = ModuleRun(
            ...
            phase="scan",
            status="queued",
            schema_id="radimorphic_result_v1",
            job_contract_schema_id="radimorphic_engine_job_v1",
        )
    ...
    async def get_job_record(self, *, tenant_id: UUID, job_id: UUID) -> ModuleRun | None:
        """Return a tenant-scoped RadiMorphic job record by ModuleRun id."""
...
class RadiMorphicWorkerRuntimeService:
    """Process queued RadiMorphic jobs using CPU-only execution."""

    def __init__(
        self,
        session: AsyncSession,
        *,
        engine: RadiMorphicEngine | None = None,
    ) -> None:
        self.session = session
        self.engine = engine or RadiMorphicEngine()
    ...
    async def process_next_queued(
        ...
        *,
        ...
        """Process the next queued RadiMorphic job without committing."""
        statement = (
        ...
        if row is None:
            return RadiMorphicWorkerOutcome(
                status="no_queued_job",
                module_run_id=None,
                tenant_id=tenant_id,
                reason="no_queued_radimorphic_job",
            )
        ...
        try:
            await RadiMorphicResultWriter(self.session).persist_result(result=result)
        except RadiMorphicResultWriteError:
...
def job_status(module_run: ModuleRun) -> RadiMorphicJobStatus:
    """Cast a ModuleRun status into the public RadiMorphic job status type."""
